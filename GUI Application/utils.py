...
working_path = str(Path().absolute())

inception_path = working_path + '\Saved Models\inception_v3.h5'
resnet_path = working_path + '\Saved Models\\Resnet-v2.h5'
mobilenet_path = working_path + '\Saved Models\mobilenet.h5'
print("Loading Models.....")
inception_model = load_model(inception_path, custom_objects={'KerasLayer': hub.KerasLayer})
resnet_model = load_model(resnet_path, custom_objects={'KerasLayer': hub.KerasLayer})
mobilenet_model = load_model(mobilenet_path)
models = [inception_model, resnet_model, mobilenet_model]
models_name = ["Inception", "Resnet", "MobileNet"]
print("Models Loaded")
...
def predict(input_image):
    '''
    returns a predicted class, associated probability
    input_image = an image
    models = list of models    
    '''
    weights = [  0.228, 0.601,0.172]
